...
export default function GuestRedirect() {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-th-page flex flex-col items-center justify-center text-center p-6 text-primary">
        <h1 className="text-3xl font-bold mb-4">Invite Link Error</h1>
        <p className="text-secondary mb-6">{error}</p>
        <button
          onClick={() => navigate('/')}
          className="bg-th-brand hover:bg-th-brand-hover text-primary px-6 py-3 rounded-lg font-semibold"
        >
          Go to Home
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-th-page flex flex-col items-center justify-center text-center text-primary p-6">
      <h1 className="text-2xl font-bold mb-3">Joining Session…</h1>
      <p className="text-secondary">Hold tight while we connect you to the party.</p>
    </div>
  );
}
